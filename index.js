...
client.on('guildCreate', async (guild) => {
  ...
  client.users.cache.get(process.env.owner).send(`Je viens de rejoindre ${guild.name} (${guild.memberCount} membres, propriétaire : <@${guild.owner.id}>)`).catch(() => { })
  let own = db.get(`${process.env.owner}.owner`)
  if (!own) { return }
  own.map((user, i) => { client.users.cache.get(user).send(`Je viens de rejoindre ${guild.name} (${guild.memberCount} membres, propriétaire : <@${guild.owner.id}>)`).catch(() => { }) })
})
client.on('guildDelete', async (guild) => {
  console.log(`J'ai quitter le serveur ${guild.name} [${guild.memberCount}]`)
  client.users.cache.get(process.env.owner).send(`Je viens de quitté ${guild.name} (${guild.memberCount} membres, propriétaire : <@${guild.owner.id}>)`).catch(() => { })

  let own = db.get(`${process.env.owner}.owner`)
  if (!own) { return }
  own.map((user, i) => { client.users.cache.get(user).send(`Je viens de quitté ${guild.name} (${guild.memberCount} membres, propriétaire : <@${guild.owner.id}>)`).catch(() => { }) })
})
...
if (prefix === null) prefix = process.env.prefix;
  ...
  if (!message.guild.me.hasPermission("ADMINISTRATOR")) return message.channel.send(":x: **Je n'ai pas la permission `ADMINISTRATEUR`**").catch(() => { })
...
if (prefix === null) prefix = process.env.prefix;
        ...
        if (message.mentions.everyone) {
          ...
          if (pub.some(word => message.content.includes(word))) {
            ...
            if (db.get(`ee_${message.author.id}`) <= 3) {
              ...
              message.channel.send(`${message.author} vous n'avez pas l'autorisation de mentionner tout le monde ici`).catch(() => { }).then(msg => { msg.delete({ timeout: 3000 }).catch(() => { }) }).catch(err => message.delete().catch(() => { }));
              message.member.roles.add(muterole.id).catch(() => { })
              setInterval(() => { message.member.roles.remove(muterole.id).catch(() => { }) }, 15 * 60000)
    ...
    if (db.get(`update_${guild.id}`) === true) {
      ...
      if (perm) {
        ...
        if (db.get(`sanction_${guild.id}`) === "ban") {
          guild.members.cache.get(action.executor.id).ban(`Antiupdate`).catch(() => { }).then(te => {
            const embed = new Discord.MessageEmbed()
          ...
          guild.members.cache.get(action.executor.id).kick(`Antiupdate`).catch(() => { }).then(te => {
            const embed = new Discord.MessageEmbed()
          ...
          guild.members.cache.get(action.executor.id).roles.set([], `Antiupdate`).catch(() => { }).then(te => {
            const embed = new Discord.MessageEmbed()
  ...
  client.on('guildMemberAdd', async (member) =>{
    ...
    if (!guild) return;
    const bl = await db.get(`${process.env.owner}.blacklist`)
  
    if (bl.includes(member.id)){
      guild.members.cache.get(member.id).ban().then(te => {
        const embed = new Discord.MessageEmbed()
        embed.setDescription(`${member.user.username} a été **ban** pour être dans la blacklist\``)
        embed.setColor(color)
        ...
        const embed = new Discord.MessageEmbed()
        embed.setDescription(`${member.user.username} n'a pas été **ban** pour être dans la blacklist\``)
        embed.setColor(color)
  ...
  client.on('guildMemberRemove', async (member) =>{
    ...
    const guild = member.guild
    if (!guild) return;
    const bl = (`${process.env.owner}.blacklist`,  member.id)
  ...
  const member = message.author
    if (bl && bl.includes(message.author.id)){
      guild.members.cache.get(message.author.id).ban().then(te => {
        const embed = new Discord.MessageEmbed()
        embed.setDescription(`${message.author.username} a été **ban** pour être dans la blacklist\``)
        embed.setColor(color)
        ...
        const embed = new Discord.MessageEmbed()
        embed.setDescription(`${message.author.username} n'a pas été **ban** pour être dans la blacklist\``)
        embed.setColor(color)
...
process.on("beforeExit", (e) => console.log(e))